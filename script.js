...
class EarlyBirdBadge {
  constructor() {
    this.targetTime = new Date('Sep 15, 2025 09:50:00 GMT+0530').getTime();
    ...
    this.eventSelectionModal = document.getElementById('eventSelectionModal');
    this.form = document.getElementById('earlyBirdForm');
    this.eventForm = document.getElementById('eventSelectionForm');
    this.countdown = document.querySelector('.countdown');
    this.badgeCounter = this.getBadgeCounter();
  ...
  init() {
    ...
    this.countdown.addEventListener('click', this.handleCountdownClick.bind(this));
    ...
    if (this.eventForm) {
      this.eventForm.addEventListener('submit', this.handleEventSelection.bind(this));
    }

    document.querySelectorAll('.early-bird-close, .badge-close, .event-close').forEach(btn => {
      btn.addEventListener('click', this.closeModals.bind(this));
    });

    const downloadBtn = document.getElementById('downloadBadge');
    const shareBtn = document.getElementById('shareBadge');

    if (downloadBtn) downloadBtn.addEventListener('click', this.downloadBadge.bind(this));
    if (shareBtn) shareBtn.addEventListener('click', this.shareBadge.bind(this));

    [this.modal, this.badgeModal, this.eventSelectionModal].forEach(modal => {
      if (modal) {
  ...
  showExistingBadge() {
    try {
      const badgeData = JSON.parse(localStorage.getItem('urja_early_bird_badge'));
      if (badgeData && badgeData.name && badgeData.code) {
        const nameEl = document.getElementById('badgeName');
        const codeEl = document.getElementById('badgeCode');
        const eventEl = document.getElementById('badgeEvent');

        if (nameEl) nameEl.textContent = badgeData.name;
        if (codeEl) codeEl.textContent = badgeData.code;
        if (eventEl) eventEl.textContent = badgeData.selectedEvent || 'Event Selected';
      ...
      alert('Error loading your badge. Please try again.');
    }
  }

  showTimeInfo(currentTime) {
    ...
    if (timeToTarget > 0) {
      ...
      message = `Early Bird reward unlocks in ${days} days, ${hours} hours! Click exactly at 9:30 AM on Sep 19, 2025.`;
    } else {
      message = 'Early Bird window has passed! But don\'t worry, you can still enjoy all URJA events.';
    }
  ...
  showEventSelection() {
    if (!this.eventSelectionModal) return;
    if (this.modal) {
      this.modal.classList.remove('active');
      this.modal.setAttribute('aria-hidden', 'true');
    }
    this.eventSelectionModal.classList.add('active');
    this.eventSelectionModal.setAttribute('aria-hidden', 'false');
  }

  handleEventSelection(e) {
    e.preventDefault();
    const selectedEvent = document.querySelector('input[name="selectedEvent"]:checked');
    if (!selectedEvent) {
      alert('Please select one event for your free access!');
      return;
    }
    this.tempUserData.selectedEvent = selectedEvent.value;
    this.createBadge(this.tempUserData.name, this.tempUserData.email, this.tempUserData.selectedEvent);
  }

  createBadge(name, email, selectedEvent) {
    const badgeData = {
      name: name,
      email: email,
      ...
      timestamp: Date.now(),
      id: this.badgeCounter
    };

    try {
      localStorage.setItem('urja_early_bird_badge', JSON.stringify(badgeData));
      ...
      if (codeEl) codeEl.textContent = badgeData.code;
      if (eventEl) eventEl.textContent = selectedEvent;

      if (this.eventSelectionModal) {
        this.eventSelectionModal.classList.remove('active');
        this.eventSelectionModal.setAttribute('aria-hidden', 'true');
      }
      ...
      this.setBadgeCounter(this.badgeCounter + 1);
  ...
  hasEarlyBirdBadge() {
    try {
      const badge = localStorage.getItem('urja_early_bird_badge');
  ...
  generateBadgeCode() {
    const prefix = 'EB';
    const year = '2025';
    const number = this.badgeCounter.toString().padStart(3, '0');
    return `${prefix}-${year}-${number}`;
  }

  getBadgeCounter() {
    try {
      return parseInt(localStorage.getItem('urja_badge_counter') || '1');
    } catch (error) {
      console.error('Error getting badge counter:', error);
      return 1;
    }
  }
  ...
  setBadgeCounter(count) {
    try {
      ...
      localStorage.setItem('urja_badge_counter', count.toString());
    } catch (error) {
      console.error('Error setting badge counter:', error);
    }
  }
  ...
  downloadBadge() {
    ...
    if (!badge) return;
    alert('To save your badge:\n\n1. Right-click on the badge\n2. Select "Save image as..." or "Copy image"\n3. Or take a screenshot of this modal\n\nYour badge is saved in your browser and you can access it anytime by clicking the countdown!');
  }
  ...
  shareBadge() {
    try {
      const badgeData = JSON.parse(localStorage.getItem('urja_early_bird_badge'));
      if (!badgeData) return;
      const text = `I got an Early Bird badge for URJA 2K25! Free access to all events. Badge: ${badgeData.code} #URJA2K25`;
      if (navigator.share) {
        navigator.share({
          title: 'URJA 2K25 Early Bird Badge',
          text: text,
      ...
      alert('Error sharing badge. Please try again.');
    }
  }
  ...
  fallbackCopy(text) {
    ...
    try {
      ...
      alert('Could not copy to clipboard. Please copy this text manually:\n\n' + text);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.querySelector('.countdown')) {
    try { new EarlyBirdBadge(); } catch (e) { console.warn(e); }
  }
});
